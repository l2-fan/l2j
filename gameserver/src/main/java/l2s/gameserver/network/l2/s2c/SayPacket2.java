...
public class SayPacket2 extends NpcStringContainer
{
	...
	public SayPacket2(int objectId, ChatType type, int isLocSharing, String charName, NpcString npcString, String... params)
	{
		...
		final int rank = RankManager.getInstance().getPlayerGlobalRank(objectId);
		if (rank == 1) {
			this.rank = 1;
		} else if (rank <= 30) {
			this.rank = 2;
	...
	protected final void writeImpl()
	{
		...
		if (_isLocSharing == 1)
		{
			if (player.getInventory().destroyItemByItemId(ItemTemplate.ITEM_ID_MONEY_L, Config.SHARE_POSITION_COST))
			{
