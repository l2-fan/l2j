...
import l2s.gameserver.network.l2.s2c.noname.ExDroppedItemLoc;
...
import l2s.gameserver.network.l2.s2c.pvpbook.ExPvpbookNewPk;
...
public final class Player extends Playable implements PlayerGroup
{
	...
	public final int MAX_SAYHAS_GRACE_POINTS = 3500000;
	...
	public void addExpAndCheckBonus(MonsterInstance mob, final double noRateExp, double noRateSp)
	{
		...
		if(noRateExp > 0)
		{
			...
			if(!(getVarBoolean("NoExp") && getExp() == Experience.getExpForLevel(getLevel() + 1) - 1))
			{
				...
				points *= Config.ALT_SAYHAS_GRACE_CONSUME_RATE;
				points = (int) getStat().calc(Stats.SAYHAS_GRACE_CONSUME, points);
	...
	public final void doPurePk(final Player killer)
	{
		...
		killer.decreaseKarma(Config.KARMA_MIN_KARMA);
	...
	protected void doPKPVPManage(Creature killer)
	{
		...
		if(killer.isPlayer() || killer instanceof FakePlayer) //addon if killer is clone instance should do also this method.
		{
			...
			if(getPvpFlag() > 0 || isInPvpZone || war || castle != null && castle.getResidenceSide() == ResidenceSide.DARK)
			{
				...
				int lvlDiff = pk.getPlayer().getLevel() - getLevel();
				if ((lvlDiff < 10) && (lvlDiff > -10))
				{
					if ((pk.getLevel() >= 40) && (getLevel() >= 40) 
							&& (pk.getClassLevel().ordinal() >= ClassLevel.SECOND.ordinal())
							&& (getClassLevel().ordinal() >= ClassLevel.SECOND.ordinal()))
						updatePvpRanking(pk);
				}
			}
			else
			...
			if(getReflection().isMain() && !isInPvpZone) {
				PvpbookInfo pvpbookInfo = getPvpbook().addInfo(this, pk, (int) TimeUnit.MILLISECONDS.toSeconds(System.currentTimeMillis()), 0);
				if(pvpbookInfo != null)
		...
		boolean isPvP = killer.isPlayable();
		...
		List<ItemInstance> drop = new LazyArrayList<ItemInstance>(), // общий массив с результатами выбора
				dropItem = new LazyArrayList<ItemInstance>(), itemToRestore = new LazyArrayList<ItemInstance>(); // временные
		...
		try
		{
			for(ItemInstance item : getInventory().getItems())
			{
				...
				if(item.getTemplate().getType2() == ItemTemplate.TYPE2_WEAPON
						|| item.getTemplate().getType2() == ItemTemplate.TYPE2_SHIELD_ARMOR
			...
			for(ItemInstance item : drop)
			{
				...
				broadcastPacket(new ExDroppedItemLoc(Location.findAroundPosition(this, Config.KARMA_RANDOM_DROP_LOCATION_LIMIT), item.getItemId()));
				if (itemToRestore.size() < 10)
					itemToRestore.add(item);
				else
				{
					ItemInstance itm = itemToRestore.get(0);
			...
			for (ItemInstance item : itemToRestore)
			{
			...
			try
			{
				...
				statement = con.prepareStatement(//
						"UPDATE characters SET face=?,beautyFace=?,hairStyle=?,beautyHairStyle=?,hairColor=?,beautyHairColor=?,sex=?,x=?,y=?,z=?" + //
								...
								"onlinetime=?,pledge_type=?,pledge_rank=?,lvl_joined_academy=?,apprentice=?,key_bindings=?,pcBangPoints=?,char_name=?,fame=?,bookmarks=?,bot_rating=?,used_world_chat_points=?,hide_head_accessories=?,craftingPoints=?,craftGaugePoints=?,honor_coins=?,last_login=? WHERE obj_Id=? LIMIT 1");
				...
				statement.setInt(39, getHonorCoins());
				statement.setLong(40, System.currentTimeMillis());
	...
	private void updatePvpRanking(Player killer)
	{
		...
		if (killer.getPlayer().getLevel() < getLevel())
		{
			killerPoints += 11;
		}
		Map<Integer, StatsSet> players = RankManager.getInstance().getPvpRankList();
	...
	public void updateUserBonus()
	{
		...
		sendPacket(new ExVitalExInfo(sgBonus, activeBonuses+passiveBonuses));
		sendUserInfo();
	}
	...
	{
		if (getActiveSubClass() != null)
			getActiveSubClass().setSayhasGrace(val);
	}
	
	public void setSayhasGrace(int val, boolean send)
	{
		if (getActiveSubClass() != null)
			getActiveSubClass().setSayhasGrace(val);
	...
	public void addKilledMob()
	{
		...
		if (Config.CLAN_KILLED_MOBS_TO_POINT <= _killedMobs)
		{
			...
			int weeklyContribution = getVarInt(PlayerVariables.WEEKLY_CONTRIBUTION, 0) + 1;
			int totalContribution = getVarInt(PlayerVariables.TOTAL_CONTRIBUTION, 0) + 1;
